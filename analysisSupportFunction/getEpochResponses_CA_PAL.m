...
for i=1:L
    ...
    if i==1 %some stuff we only need to do once
        ...
        poststimIntervalLen = max(xvals)-intervalEnd;%s
    end
...
%     end;
    ...
    %DT get poststimulus firing rate
    poststimSpikeTimes = spikeTimes(spikeTimes > intervalEnd);
    outputStruct.poststimRate.value(i) = length(poststimSpikeTimes) / poststimIntervalLen;
